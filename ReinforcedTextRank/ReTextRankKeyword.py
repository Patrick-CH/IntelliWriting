...
def get_default_tf_idf_file():
    d = os.path.dirname(os.path.realpath(__file__))
    return os.path.join(d, 'tf-idf.json')


class ReTextRankKeyword(object):

    def __init__(self, stop_words_file=None, use_tf_idf=True,
                 allow_speech_tags=util.allow_speech_tags,
                 delimiters=util.sentence_delimiters):
        """
        Keyword arguments:
        stop_words_file  --  str，指定停止词文件路径（一行一个停止词），若为其他类型，则使用默认停止词文件
        td_idf_file      --  str, 指定 TF-IDF 数据文件路径
        delimiters       --  默认值是`?!;？！。；…\n`，用来将文本拆分为句子。
        ...
        self.tf_idf = None
        if use_tf_idf:
            self.tf_idf = json.load(open(get_default_tf_idf_file(), 'r', encoding='utf-8'))
