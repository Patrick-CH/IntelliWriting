...
def set_args():
    ...
    parser.add_argument('--model_path', default='trained_model/model0', type=str, help='模型文件路径')
    ...
    parser.add_argument('--top_p', default=0.97, type=float, help='解码时保留概率累加大于多少的标记')
...
def generate(input_text):
    ...
    model.to(device)
    model.eval()
    titles = predict_one_sample(model, tokenizer, device, args, input_text)
    return titles        
